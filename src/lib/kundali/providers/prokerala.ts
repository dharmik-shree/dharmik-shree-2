...
export class ProkeralaProvider implements AstrologyProvider {
  ...
  private async fetchEndpoint(endpoint: string, params: Record<string, string>): Promise<any> {
    ...
    if (!res.ok) {
      const errText = await res.text();
      console.warn(`Prokerala API endpoint '${endpoint}' returned ${res.status}:`, errText);
      return null;
    }
  ...
  async fetchFullKundaliData(birth: BirthDetails): Promise<ProviderRawData> {
    ...
    // Parallel endpoint fetches
    const [kundli, panchang, d1Chart, d9Chart, chalitChart, mangalDosha, sadeSati, kaalSarp, vimshottariDasha] =
      await Promise.all([
        ...
        this.fetchEndpoint("chart", { ...baseParams, chart_type: "chalit", chart_style: "north-indian" }),
        ...
        this.fetchEndpoint("vimshottari-dasha", baseParams),
      ]);
